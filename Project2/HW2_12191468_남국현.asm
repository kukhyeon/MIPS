...
main:    
    ...
    move	$a0, $s5			    # 시작 주소
    addi	$a1, $s6, -4		    # 끝 주소
    add	    $s0, $zero, $zero       # Left 포인터 초기화 
    add	    $s1, $zero, $zero       # Right 포인터 초기화
    add	    $s2, $zero, $zero       # Pivot 초기화
...
swap:
    ...
    # 포인터 이동
    addi    $t1, $t1, 4       # &Left += 4
    addi    $t2, $t2, -4      # &Rgiht -= 4

    j       partition_loop

# Left part 부터 퀵소팅하고 right part 퀵소팅
partition_end:
    # Left 부분 배열 정렬
    blt     $s0, $t2, quick_left_sort
    # 시작 주소 >= 끝 주소이면 재귀 호출 생략(원소가 pivot하나)
    j       check_right_subarray
quick_left_sort:
    # &s0은 이전 루프의 첫 주소를 담고 있지만 $t2는 &left와 위치가 크로스된 &right
    move    $a0, $s0          # 시작 주소
    move    $a1, $t2          # 끝 주소
    jal     quick_sort        # left 재귀 시작
...
check_right_subarray:
    ...
    j       quick_sort_end
quick_right_sort:
    # &s1은 이전 루프의 끝 주소를 담고 있지만 $t1는 &right와 위치가 크로스된 &left
    move    $a0, $t1          # 시작 주소
    move    $a1, $s1          # 끝 주소
    jal     quick_sort        # right 재귀 시작
